...
if (new Date - user.lastmiming < 600000) return await conn.reply(m.chat, `*⏱️ ارجع للداخل ${msToTime(time - new Date())} لمواصلة الرحله ${global.rpgshopp.emoticon('exp')}⚡*\n\n*نعود للداخل ${msToTime(time - new Date())} لي ${global.rpgshopp.emoticon('exp')}⚡*`, fkontak,  m)
...
handler.help = ['minar']
handler.tags = ['xp']
handler.command = ['الرحلة', 'راحله', 'راحلة', 'رحلة', 'الرحله', 'رحله'] 
handler.fail = null
handler.exp = 0
